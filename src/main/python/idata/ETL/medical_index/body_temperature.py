...
f = open("D:\\工作\\2023\\3月\\省立医院数抽样\\指标\\sample_body_temperature.rds")
...
# 创建表
table_name = 'medical_body_temperature'
...
for index in range(len(lin) - 1):
    ...
    print(ID,person_id_new, seq_id, sign_value_new,
          sign_name,time,source,unit_new,date)

    if(ID != ""):
        cursor.execute("INSERT INTO medical_body_temperature("
                       "id,"
                       "person_id_new,"
                       "seq_id,"
                       "sign_value_new,"
